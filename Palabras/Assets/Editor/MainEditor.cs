...
public class MainEditor : EditorWindow {
    ...
    void OnGUI() {
        ...
        if(GUILayout.Button("Cartas", GUILayout.Height(45), GUILayout.Width(100))) {
            modulo = (IModulo) carta;
        }
    ...
    T[] CopiarValor<T> (T[] array, int value) where T:IModulo{
        if(value < 0 || value >= array.Length)
            return array;
        ...
        nuevoArray[array.Length] = list.ToArray()[0];
